...
try {
  ...
  formatGelfMessage = gf.formatGelfMessage || gf.default?.formatGelfMessage
} catch (_e) {
...
const STATIC_META = process.env.GRAYLOG_STATICMETA
  ? JSON.parse(process.env.GRAYLOG_STATICMETA)
  : {}
...
function connect() {
  ...
  if (PROTOCOL === 'tcp') {
    ...
    socket = tls.connect(
      { host: HOST, port: PORT, rejectUnauthorized: true },
      onConnect,
    )
  }
  ...
  socket.on('error', (err) => {
    ...
    if (process.send)
      process.send({ type: 'child-error', message: String(err) })
  })
  ...
  if (msg.type === 'log') {
    try {
      ...
      if (formatGelfMessage) {
        ...
        gelf = JSON.stringify({
          short_message:
            typeof msg.payload === 'object' ? msg.payload : String(msg.payload),
          host: HOSTNAME,
          _facility: FACILITY,
          ...STATIC_META,
        })
