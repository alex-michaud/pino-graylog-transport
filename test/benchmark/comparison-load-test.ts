...
for (let i = 0; i < 100; i++) {
  largeMetadata[`field_${i}`] = `value_${i}_${Math.random().toString(36).substring(7)}`;
}
...
export function setup() {
  ...
  if (healthCheck.status !== 200) {
    console.error('Server health check failed! Is the comparison server running?');
...
export function teardown() {
  ...
  console.log('  - pino_duration:     Response time with pino-graylog-transport');
  console.log('  - winston_duration:  Response time with winston + winston-graylog2');
  console.log('');
  console.log('Lower duration = better performance');
  console.log('The difference between pino/winston and baseline shows logging overhead.');
  console.log('');
  console.log('Check Graylog UI at http://localhost:9005 to verify logs were received.');
}
